...
log_name = 'optimism.log'

# wait up to 20 minutes
delay = random.randint(1, 20) * random.randint(5, 30) * random.randint(1, 2)
#sleep(delay)
print(f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} Started Optimism job')

web3 = Web3(Web3.HTTPProvider(rpc.provider['optimism']))
...
times = random.randint(1, 2) + random.randint(0, 4)
